...
$(document).ready(function () {
  ...
  $(".our-works__slider_content").on(
    "beforeChange",
    function (event, slick, currentSlide, nextSlide) {
      let current = $(
        ".our-works__slider_content .slick-slide[data-slick-index =" +
          nextSlide +
          "]"
      ).attr("data-image");
  ...
  /******************/
  $(".header_form_send").click(function (e) {
    e.preventDefault();
    if ($("#header_form_name").val() == "") {
      alert("Заполните поле имя");
      return;
    }
    if ($("#header_form_phone").val() == "") {
      alert("Заполните поле телефон");
      return;
    }
    let phone = $("#header_form_phone").val();
    let name = $("#header_form_name").val();
    let address = $("#header_form_address option:selected").val();
    if (!address) {
        address = $("#header_form_address").val();
    }
  ...
  $(".popup1_send").click(function (e) {
    ...
    // let address = $("#popup1_address").val();
    let address = $("#popup1_address option:selected").val();
    if (!address) {
        address = $("#popup1_address").val();
    }
...
$(".indigo_quiz_card").click(function () {
  indigo_quiz_select[$("#page_" + indigo_quiz_step).attr("data-title")] =
...
$("#indigo_quiz_submit").click(function () {
  ...
  if (
    $("#indigo_quiz_name").val().length != 0 &&
    $("#indigo_quiz_phone").val().length == 16
  ) {
    hidden_page(indigo_quiz_step);
    indigo_quiz_step++;
    show_page(indigo_quiz_step);
...
function show_page(e) {
  if (e <= indigo_quiz_page) {
    ...
    setTimeout(
      () => $("#page_" + e).removeClass("indigo_quiz_page_hidden"),
      500
    );
    setTimeout(
      () => $("#header_" + e).removeClass("indigo_quiz_page_hidden"),
      500
    );
  } else {
    $("#page_" + e).removeClass("indigo_quiz_page_opacity");
    $("#header_" + e).removeClass("indigo_quiz_page_opacity");
    setTimeout(
      () => $("#page_thanks").removeClass("indigo_quiz_page_hidden"),
      500
    );
    // setTimeout(() => $('.indigo_quiz').addClass('indigo_quiz_page_opacity'), 2000);
    setTimeout(
      () => $(".indigo_quiz").addClass("indigo_quiz_page_scroll"),
      2500
    );
    setTimeout(
      () => $(".indigo_quiz_body").addClass("indigo_quiz_page_scroll"),
      2500
    );
    setTimeout(
      () => $(".indigo_quiz_button").addClass("indigo_quiz_page_opacity"),
      2000
    );
    setTimeout(
      () => $(".indigo_quiz_button").addClass("indigo_quiz_button_hidden"),
      2500
    );
...
(function (w, d, s, h, id) {
  ...
  "script",
  "cloud.roistat.com",
  "0dc92382b437fa129ca715bc94304147"
);

window.onRoistatAllModulesLoaded = function () {
  roistat.leadHunter.onBeforeSubmit = function (lead) {
    lead.isNeedCallback = 1;
    lead.callbackPhone = 74993911874;
  };
};
/* Roistat END */
...
document.addEventListener("DOMContentLoaded", function () {
  setTimeout(function () {
    loadScript(
      "https://api-maps.yandex.ru/services/constructor/1.0/js/?um=constructor%3Ade10cd703999873b83a1849058be30acc9f9e1c8c106d81832972f69b5822ae5&amp;width=100%25&amp;height=550&amp;lang=ru_RU&amp;scroll=true",
      //"https://api-maps.yandex.ru/services/constructor/1.0/js/?um=constructor%3A04a8e00d3d756f3bb8c985c50f72712a73a94044c9272a1820b2c96e3a5ab3e3&amp;width=100%25&amp;height=550&amp;lang=ru_RU&amp;scroll=true",
...
$(".workshop-area").slick({
  ...
  responsive: [
    ...
    {
      breakpoint: 992,
      settings: { slidesToShow: 2, slidesToScroll: 2, arrows: !1, dots: !0 },
    },
    {
      breakpoint: 768,
      settings: { slidesToShow: 1, slidesToScroll: 1, arrows: !0, dots: !1 },
    },
  ],
});

function initTeamSlider() {
  if ($(window).width() < 768) {  // мобильные
    if (!$('.slider-team').hasClass('slick-initialized')) {
      $('.slider-team').slick({
        slidesToShow: 1,
        slidesToScroll: 1,
        arrows: true,
    ...
    if ($('.slider-team').hasClass('slick-initialized')) {
      $('.slider-team').slick('unslick'); // отключаем на десктопе
    }
  }
}

$(document).ready(initTeamSlider);
$(window).resize(initTeamSlider);
